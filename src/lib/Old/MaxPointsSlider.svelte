<script lang="ts">    
    ...
    onMount(() => {
        if ($plotStore.args?.max_points) {
            // Subscribe to the plotStore
           
        }
    })

    $: if (plotStore) {
        const unsubscribe = plotStore.subscribe(value => {
            // Update max whenever the store's state changes
            max = value.meta?.n_points || 1000;  // Default value if n_points is not defined
        });
    ...
    const handleSliderChange = () => {
        plotStore.updateMaxPoints(lower)
